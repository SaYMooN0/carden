...
type PlantsRepository() =
    ...
    member _.UpsertCardForPlantOwner
        (conn: NpgsqlConnection)
        (ownerId: AppUserId)
        (plantId: PlantId)
        (req: ParsedUpsertPlantCardRequest)
        : Task<Result<UpsertPlantCardDbResult option, string>> =
        task {
            let contentArraySql (jsonParamName: string) =
                $"""
                ARRAY(
                    SELECT ROW(x."type", x."text", x."image", x."expression")::content_item
                    FROM jsonb_to_recordset(CAST(@{jsonParamName} AS jsonb))
                        AS x("type" text, "text" text, "image" text, "expression" text)
                )
                """

            let contentFrontSql = contentArraySql "ContentFrontJson"
            let contentBackSql = contentArraySql "ContentBackJson"
            ...
            try
                ...
                if deckId = Guid.Empty then
                    do! tx.RollbackAsync()
                    return Ok None
                else
                    let now = DateTimeOffset.UtcNow

                    let! existingCardIdInDeck =
                        match req.CardId with
                    ...
                    let! savedCard =
                        if existingCardIdInDeck then
                            let updateSql =
                                $"""
                                UPDATE card
                                SET
                                    "ContentFront" = {contentFrontSql},
                                    "ContentBack" = {contentBackSql},
                                    "LastTimeEdited" = @Now
                                WHERE "Id" = @CardId AND "DeckId" = @DeckId
                                RETURNING "Id", "LastTimeEdited", "CreationTime"
                                """

                            task {
                                let cardId = req.CardId.Value

                                let! dto =
                                    conn.QuerySingleAsync<SavedCardDbDto>(
                                        updateSql,
                                        {| CardId = cardId
                                           DeckId = deckId
                                           Now = now
                                           ContentFrontJson = req.ContentFrontJson
                                           ContentBackJson = req.ContentBackJson |},
                                        tx
                                    )

                                return
                                    { Id = dto.Id
                                      LastTimeEdited = dto.LastTimeEdited
                            ...
                            let insertSql =
                                ...
                                VALUES
                                    (@Id, @DeckId, {contentFrontSql}, {contentBackSql}, @Now, @Now)
                                RETURNING "Id", "LastTimeEdited", "CreationTime"
                                """
                    ...
                    do! tx.CommitAsync()
                    return Ok(Some savedCard)

            with ex ->
                return Error $"Upsert card failed: {ex.Message}"
        }